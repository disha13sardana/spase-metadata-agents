...
**Relevance scoping:** for a *data record*, the producing instrument's team is what matters — do not sweep in the PIs of unrelated instruments on the same observatory (note them in `instrument_coverage` and move on). For an *Observatory record*, all its instruments' leads are legitimate candidates.
...
- **Not alphabetized** (backward scan found no qualifying sorted suffix): author order signals contribution — the first author did the most work and is the most important. Take up to the first five authors, **weighted by position: authors 1–3 are Strong-tier; authors 4 and 5 are Medium-tier** (position still supports them, but less decisively). Do not take authors beyond the fifth on position alone. Be aware that some instrument-paper authors may not appear on any other science paper, so they can be hard to corroborate elsewhere; that does not disqualify them when author order supports them.
...
- The **4th or 5th author** (or 4th/5th prefix position) of a non-alphabetized or partially alphabetized description paper, when paper selection is confident
...
- A non-lead / uncorroborated co-author (especially from an alphabetized author list or the alphabetized tail of a partially alphabetized one)
